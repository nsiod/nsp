...
// `mock-control` container (compose). The mock returns whatever
// snapshot the test programs into `/__test__/snapshot`, and
// records every reverse-API request the node makes
// (`/__test__/captures`).
//
// Scenarios:
//   1. /config drives the local DB — Full snapshot with users +
//      iptables → user/rule appear locally with `source = control`.
//   2. /config delta — `users.delete` removes the row.
//   3. Source boundary — control upsert collides with a local
//      user → local user untouched, /report event observed.
//   4. /status heartbeat — periodic POST with services + traffic.
//
// The mock starts in "empty snapshot" mode so other phases (which
// run before 13 alphabetically) are not perturbed by this loop.
//
// Cadence: NSP_CONTROL_INTERVAL_SECS=2 in compose. Tests give the
// node ~10s to converge, well above the loop period.
...
const MOCK_BASE = process.env["MOCK_CONTROL_BASE"] ?? "http://mock-control:9090";
const NODE_ID = process.env["MOCK_CONTROL_NODE_ID"] ?? "node-e2e";
const TICK_MS =
  Number(process.env["NSP_CONTROL_INTERVAL_SECS"] ?? "2") * 1000;
...
beforeAll(async () => {
  ...
  localCollisionId = `shared-${suffix}`;
  await clearSnapshot();
  await clearCaptures();
});
...
afterAll(async () => {
  ...
  // control-source rows we orphan in the DB but the next snapshot
  // clears their iptables rules.
  await clearSnapshot();
  await clearCaptures();
  // Local user is admin-deletable; control-source rows we leave to
  // the compose teardown (the API would 403 those by design).
  if (localCollisionId) {
    await client.status("DELETE", `/api/users/${localCollisionId}`);
  }
});

describe("phase 13 — reverse-API control center", () => {
  ...
  test("empty iptables list under default policy keeps existing control rules", async () => {
    // Default conflict_policy is `keep`. An iptables: [] payload
    // means "the server has nothing to add" — pre-existing
    // control-source rules should survive.
    await setSnapshot({
      cursor: "v5",
      users: [
        { id: controlUserA, name: `ctl-alice-${suffix}`, note: "from control" },
    ...
    // Give a couple of ticks for the reconcile to land.
  ...
  test("mode=replace prunes control-source rules absent from snapshot", async () => {
    // Server-driven hard alignment overrides the operator's
    // policy=keep default.
    await setSnapshot({
      cursor: "v6",
      mode: "replace",
    ...
    await waitUntil(
      ...
      { label: "mode=replace evicts the control-source rule" },
    );
  });
});
